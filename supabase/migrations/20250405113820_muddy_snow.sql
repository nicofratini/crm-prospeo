...
-- Add unique constraint for ai_agents if not exists
DO $$ BEGIN
  IF NOT EXISTS (
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'call_history_status_check'
  ) THEN
    ALTER TABLE public.call_history 
    ADD CONSTRAINT call_history_status_check 
    CHECK (status = ANY (ARRAY['completed', 'missed', 'failed', 'in-progress']));
  END IF;
END $$;
...
-- Add unique constraint for tag names per user if not exists
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
